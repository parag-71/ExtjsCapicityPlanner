...
Ext.define('LeankorApp.view.MainViewport', {
	...
	initComponent: function () {
		...
		// We use outerWidth as a stable reference for the unzoomed layout width.
		...
		if (!Ext.window.Window._wcagVerticalApplied) {
			...
			Ext.override(Ext.window.Window, {
				afterRender: function () {
					...
					// Skip windows that handle their own vertical scroll
					// (card editor, preferences — they have targeted fixes)
					if (this.el && (this.el.hasCls('taskEditorPluginCls') || this.el.hasCls('taskEditorCls'))) {
						return;
					}
					var me = this;
					// Helper: constrain to viewport when zoomed
					var constrainToVh = function () {
						...
						if (pos[1] + me.getHeight() > vh) {
							me.setPosition(pos[0], Math.max(0, vh - me.getHeight()));
						}
					};
					constrainToVh();
		...
		if (btype == 'ru') {
			mainPanel = {
				...
				header: {
					xtype: 'controlheader',
					title: Ext.htmlEncode(Locale.LocaleName.CapacityPlanning)
				}
			};
