...
Ext.define('LeankorApp.Application', {
	...
	launch: function () {
		...
		var clearKeyboardFocusMode = function () {
			if (Ext.getBody()) {
				Ext.getBody().removeCls('lk-keyboard-focus-mode');
			}
		};

		Ext.getDoc().on('mousedown', clearKeyboardFocusMode);
		Ext.getDoc().on('touchstart', clearKeyboardFocusMode);
